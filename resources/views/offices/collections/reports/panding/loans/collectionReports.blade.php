        ...
        <div class="row justify-items-center">
            <div class="col-md-6">
                ...
                <p>
                    Pending Collections
                    <a target="__blank"
                        href="{{ Route('collectionsReport.pendingCollection.loans.reports.print', ['type_id' => request()->type_id, 'volume_id' => request()->volume_id, 'officer' => isset(request()->officer) ? request()->officer : '0', 'start_date' => isset(request()->startDate) ? date('Y-m-d', strtotime(request()->startDate)) : 'null', 'end_date' => isset(request()->endDate) ? date('Y-m-d', strtotime(request()->endDate)) : 'null']) }}"><span
            ...
            <div class="col-md-6 justify-content-md-between align-items-md-center d-md-flex">
                <form
                    action="{{ Route('collectionsReport.pendingCollection.loans.reports', ['type_id' => request()->type_id, 'volume_id' => request()->volume_id]) }}"
                        ...
                        <table class="table table-hover table-striped table-report">
                            ...
                            <tbody>
                                ...
                                @forelse ($loan->LoanProfile as $key => $LoanProfile)
                                    @forelse ($LoanProfile->LoanCollection as $index => $LoanCollection)
                                        @php
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <form id="update_collection"
                            action="{{ Route('collectionsReport.regularCollection.loans.reports.update') }}"
                            method="POST">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".collection-delete").on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ Route('collectionsReport.regularCollection.loans.reports.delete') }}",
                            type: "DELETE",
                            headers: {
            ...
            $(".collection-edit").on('click', function() {
                ...
                $.ajax({
                    url: "{{ Route('collectionsReport.regularCollection.loans.reports.edit') }}",
                    type: "POST",
                    headers: {
                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
                    },
