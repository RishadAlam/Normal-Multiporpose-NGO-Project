        ...
        <div class="row justify-items-center">
            <div class="col-md-8">
                ...
                <p>
                    ...
                    <a target="__blank"
                        href="{{ Route('collectionsReport.regularCollection.savings.reports.print', ['type_id' => request()->type_id, 'volume_id' => request()->volume_id, 'officer' => isset(request()->officer) ? request()->officer : '0']) }}"><span
            ...
            <div class="col-md-4">
                @if (auth()->user()->can('Collection Report View as an Admin'))
                    <form
                        action="{{ Route('collectionsReport.regularCollection.savings.reports', ['type_id' => request()->type_id, 'volume_id' => request()->volume_id]) }}"
                                ...
                                @forelse ($saving->SavingsProfile as $key => $SavingsProfile)
                                    @forelse ($SavingsProfile->SavingsCollection as $SavingsCollection)
                                        <tr>
                                            ...
                                            <td>
                                                {{ date('h:i A', strtotime($SavingsCollection->time)) }}
                                            </td>
                                            @if (auth()->user()->can('Collection Edit'))
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <form id="update_collection"
                            action="{{ Route('collectionsReport.regularCollection.savings.reports.update') }}"
                            method="POST">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".collection-delete").on('click', function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ Route('collectionsReport.regularCollection.savings.reports.delete') }}",
                            type: "DELETE",
                            headers: {
            ...
            $(".collection-edit").on('click', function() {
                ...
                $.ajax({
                    url: "{{ Route('collectionsReport.regularCollection.savings.reports.edit') }}",
                    type: "POST",
                    headers: {
                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
                    },
            ...
            $(".aprroval_submit").on('click', function(e) {
                ...
                let url = "{{ Route('collectionsReport.regularCollection.savings.reports.approve') }}"
                var id = [];
